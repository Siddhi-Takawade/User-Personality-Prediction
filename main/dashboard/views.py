...
loaded_model_1 = pickle.load(open(os.path.join(settings.BASE_DIR, "dashboard/trained_model_1.sav"),"rb"))
loaded_model_2 = pickle.load(open("C:/Users/Admin/Project folder/user_personality_prediction/main/dashboard/trained_model_2.sav","rb"))
loaded_model_3 = pickle.load(open("C:/Users/Admin/Project folder/user_personality_prediction/main/dashboard/trained_model_3.sav","rb"))
loaded_model_4 = pickle.load(open("C:/Users/Admin/Project folder/user_personality_prediction/main/dashboard/trained_model_4.sav","rb"))

postdf=pd.read_csv(os.path.join(settings.BASE_DIR, "dashboard\pickle_models\post.csv"))
...
def detailed_personality(trait) :
    ...
    if trait=="ISTJ":
        return("The Inspector:/nReserved and practical, they tend to be loyal, orderly, and traditional./nCharacteristics: Responsible, sincere, analytical, reserved, realistic, systematic. Hardworking and trustworthy with sound practical judgment.")
    elif trait=="ISTP":
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        best = str(request.POST['best'])
        bad = str(request.POST['bad'])
        ...
        if Student.objects.filter(first_name=fname, last_name=lame).exists():
            ...
            total_text = best + bad + desc 
            student_ = Student(first_name=fname, last_name=lame, email=email, phone=phone, description=total_text, gender=gender, area=add1)
            student_.save()
